describe('The ng-repeat directive', function () {
  ...
  beforeEach(inject(function($compile, _$rootScope_, $templateCache) {
    ...
    element = $compile('<ul><li id="thought" ng-repeat="thought in thoughts">{{thought.text}}</li><ul>')(scope);
    scope.$digest();
  }));
  ...
  it('should display the thought\'s text', function () {
  	expect(element.find('li')[0].textContent).toBe(thoughts[0].text);
  });
